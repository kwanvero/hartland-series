...
export default function DecoyLanding() {
  ...
  useEffect(() => {
    ...
    const metas = [
      { property: "og:title", content: "The Heartland Chronicles Manuscript" },
      {
        ...
        content: "Private · Authorised Access Only · veemercado.com",
      },
  ...
  useEffect(() => {
    ...
    const recordVisit = async () => {
      ...
      let existing = [];
      try {
        const stored = await window.storage.get("heartland-visits");
        if (stored) existing = JSON.parse(stored.value);
      } catch {}
      const updated = [...existing, geo];
      try {
        await window.storage.set("heartland-visits", JSON.stringify(updated));
      } catch {}
      setVisits(updated);
      setLoading(false);
    };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const stored = await window.storage.get("heartland-visits");
        if (stored) setVisits(JSON.parse(stored.value));
      } catch {}
    };
    load();
  }, [adminUnlocked]);
  ...
  const clearVisits = async () => {
    try {
      await window.storage.set("heartland-visits", JSON.stringify([]));
      setVisits([]);
    } catch {}
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <br />I wouldn't put ALL my ideas on-the-line (get it?). <br />
        <a
